		...
		$.ajax({
			url:"<%= request.getContextPath()%>/opendata/weatherXML.action",
	         ...
	         success:function(xml){
	        	var rootElement = $(xml).find(":root");  // var 를 준 이유는 없다. let, const 둘 다 가능
	        	 // console.log(" 확인용 :  " + $(rootElement).prop("tagName") );  //  확인용 :  current
	        	 // console.log(" 확인용 :  " + rootElement.prop("tagName") );		//  확인용 :  current
	        	 ...
	        	 // console.log(updateTime); => 2023년  06월  07일 11시
	        	 
	        	 var localArr  = rootElement.find("local");
	        	 // console.log(" 지역 개수 : "+ localArr.length);   지역 개수 : 97
		        ...
		        for ( let x=0 ;  x<localArr.length ; x ++){
		        	const local = localArr.eq(x);
		        	/* .eq(index) 는 선택된 요소들을 인덱스 번호로 찾을 수 있는 선택자이다. 
                    마치 배열의 인덱스(index)로 값(value)를 찾는 것과 같은 효과를 낸다.
	                  */
	                  
	                // console.log(local.text()); // 청주 , 속초 등등 
		        	// console.log(local.attr("desc")) ; // 구름많음 , 맑음 등등
		        	// console.log(local.text()  + " icon:" + $(local).attr("icon") + " desc:" + $(local).attr("desc") + " ta:" + $(local).attr("ta") );
